  ...
  EFFECTS.forEach((effect) => {
    ...
    Object.entries(effect.params).forEach(([paramKey, config]) => {
      if (config.type === "toggle") {
        ...
        valueLabel.textContent = formatSliderValue(config.default, config.step);
        ...
        input.addEventListener("input", () => {
          ...
          valueLabel.textContent = formatSliderValue(value, config.step);
        ...
        updateEffectParam(effect.id, paramKey, config.default);
      }
    });
...
async function exportVideo() {
  ...
  video.loop = false;
  const durationSeconds = exportDurationInput ? parseFloat(exportDurationInput.value) : 6;
  const limitedDuration = Number.isFinite(durationSeconds) ? Math.max(1, durationSeconds) : 6;
  ...
  exportTimeoutId = setTimeout(stopRecording, limitedDuration * 1000);
}
...
function setupControls() {
  ...
  if (exportDurationInput && exportDurationValue) {
    const updateLabel = () => {
      const value = parseFloat(exportDurationInput.value);
      const clamped = Number.isFinite(value) ? Math.max(1, value) : 6;
      exportDurationValue.textContent = `${Math.round(clamped)}s`;
    };
